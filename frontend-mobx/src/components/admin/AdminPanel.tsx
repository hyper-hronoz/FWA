...
const AdminPanel = observer(function AdminPanel() {
  ...
  const profiles = store.adminChans;
  const loadingProfiles = store.adminLoading;
  ...
  useEffect(() => {
    void store.fetchAdminChans(false);
  }, [store]);
  ...
  const handleAdd = async () => {
    ...
    try {
      await store.createGirlAdmin(formData);
      cleanupForm();
      await store.fetchAdminChans(true);
      alert("Тянка успешно добавлена!");
    } catch (err: unknown) {
      console.error("Ошибка добавления тянки:", err);
      const msg = err instanceof Error ? err.message : "неизвестная ошибка";
      alert("Не удалось добавить тянку: " + msg);
    }
  };
  ...
  const handleSave = async () => {
    ...
    try {
      await store.updateGirlAdmin(editingId, formData);
      cleanupForm();
      await store.fetchAdminChans(true);
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      await store.deleteGirlAdmin(id);
      await store.fetchAdminChans(true);
    } catch (error: unknown) {
      console.error("Ошибка удаления тянки:", error);
      const msg = error instanceof Error ? error.message : "неизвестная ошибка";
      alert("Не удалось удалить тянку: " + msg);
    }
  };
