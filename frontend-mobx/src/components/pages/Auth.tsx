...
const Auth = observer(function Auth({ mode }: AuthProps) {
  ...
  if (store.authPending && !store.user) return <AnimeLoading />
  if (store.user) return <Navigate to="/swipe" replace />
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!isLogin) {
      ...
      if (!result.success) {
        setError(result.error || "Произошла неизвестная ошибка при входе")
        return
      }
