...
export default function AdminPanel() {
  ...
  const { data: profiles = [], isLoading: loadingProfiles, isError } = useGetAdminCatalogQuery();
  const [createChan, { isLoading: creating }] = useCreateChanAdminMutation();
  const [updateChan, { isLoading: updating }] = useUpdateChanAdminMutation();
  const [deleteChan, { isLoading: deleting }] = useDeleteChanAdminMutation();
  ...
  const handleAdd = async () => {
    ...
    try {
      await createChan(formData).unwrap();
      cleanupForm();
      ...
      const msg = err && typeof err === "object" && "data" in err
        ...
        : "";
      alert("Не удалось добавить тянку: " + (msg || "неизвестная ошибка"));
    }
  };
  ...
  const handleSave = async () => {
    ...
    try {
      await updateChan({ id: editingId, body: formData }).unwrap();
      cleanupForm();
      ...
      const msg = error && typeof error === "object" && "data" in error
        ...
        : "";
      alert("Не удалось сохранить изменения: " + (msg || "неизвестная ошибка"));
    }
  };
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      await deleteChan(id).unwrap();
    } catch (error: unknown) {
      console.error("Ошибка удаления тянки:", error);
      const msg = error && typeof error === "object" && "data" in error
        ? String((error as { data?: unknown }).data)
        : "";
      alert("Не удалось удалить тянку: " + (msg || "неизвестная ошибка"));
    }
  };
          ...
          <div className="bg-white/8 backdrop-blur-md rounded-2xl p-6 border border-pink-500/30 mb-10 shadow-xl">
            ...
            <div className="flex flex-col sm:flex-row justify-end gap-4 mt-8">
              ...
              <button
                ...
                disabled={uploading !== null || !isFormValid() || creating || updating}
